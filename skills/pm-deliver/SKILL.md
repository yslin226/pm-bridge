...
4. No default branch exists, or the user is already on it (single-branch repo, or committing straight to `main`/`master`) — read the most recent file in `docs/pm/delivered/` and look for a `<!-- pm-bridge: diff-range <sha>..<sha> -->` comment at its top; use `<end-sha>..HEAD` as this run's range. Verify the sha still resolves (`git rev-parse --verify <sha>^{commit}`) — rebase/squash/history rewrites can invalidate it. If the sha does not resolve, or no such comment or file exists, fall back to the last commit on the branch.
...
- **Match found**: load its acceptance criteria (驗收條件). For each criterion, determine from the diff whether it is 完成 / 部分完成 / 未動工. Write the results back into the requirement file: check the checkbox for 完成; for 部分完成 keep the checkbox unchecked and append 「（部分完成：一句說明）」 to the end of that line. If ALL criteria pass, set the requirement's status field to `delivered` and update its 最後更新 date to today.
...
狀態 field: if neither the diff nor the user establishes the deployment state, fill 「開發完成待部署（預設值）」 — marked as a default, not asserted as fact. Do not spend one of the two questions on it.
...
Save to `docs/pm/delivered/YYYY-MM-DD-<slug>.md` where `<slug>` is a short kebab-case feature name. Create directories as needed. On the first line of the file, before the title, write `<!-- pm-bridge: diff-range <start-sha>..<end-sha> -->` recording the exact commit range resolved in step 1 — it is the provenance record and the starting point for the next `/pm-deliver` run (see step 1.4). Show the user the full document and where it was saved.
...
- No hype adjectives: 強大、大幅、顯著、完美、無縫 and their equivalents in the output language are banned unless backed by a number in the same sentence.
